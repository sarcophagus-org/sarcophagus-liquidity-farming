...
contract("LiquidityFarming", (accounts) => {
  ...
  beforeEach(async function () {
    ...
    this.liquidityFarming = await LiquidityFarming.new(this.lp.address, this.sarco.address)
  });

  it("has expected LP address", async function () {
    expect(await this.liquidityFarming.lp()).to.equal(this.lp.address);
  });

  it("has expected Sarco address", async function () {
    expect(await this.liquidityFarming.sarco()).to.equal(this.sarco.address);
  });

  it("has expected owner", async function () {
    expect(await this.liquidityFarming.owner()).to.equal(owner);
  });
  ...
  context("deposit reward tokens", function () {
    ...
    beforeEach(async function () {
      ...
      await this.sarco.transfer(this.liquidityFarming.address, totalRewards);
      tx = await this.liquidityFarming.deposit(totalRewards, start, end);
    });
    ...
    it("has expected SARCO balance", async function () {
      expect(await this.sarco.balanceOf(this.liquidityFarming.address)).to.be.bignumber.equal(totalRewards);
    });

    it("has expected total rewards", async function () {
      expect(await this.liquidityFarming.totalRewards()).to.be.bignumber.equal(totalRewards);
    });

    it("has expected start time", async function () {
      expect(await this.liquidityFarming.startTime()).to.be.bignumber.equal(start);
    });

    it("has expected end time", async function () {
      expect(await this.liquidityFarming.endTime()).to.be.bignumber.equal(end);
    });

    it("has expected total stake", async function () {
      expect(await this.liquidityFarming.totalStakeLp()).to.be.bignumber.equal("0");
    });

    it("has no stakers", async function () {
      expect(await this.liquidityFarming.totalStakers()).to.be.bignumber.equal("0");
    });

    it("has zero staker1 stake", async function () {
      expect(await this.liquidityFarming.userStakeLp(staker1)).to.be.bignumber.equal("0");
    });

    it("has zero staker2 stake", async function () {
      expect(await this.liquidityFarming.userStakeLp(staker2)).to.be.bignumber.equal("0");
    });

    it("cannot stake with 0 amount", async function () {
      await expectRevert(
        this.liquidityFarming.stake("0"),
        "LiquidityFarming::stake: missing LP"
      );
    });

    it("cannot stake before start time", async function () {
      await expectRevert(
        this.liquidityFarming.stake(staker1Initial),
    ...
    context("advance to start time", function () {
      ...
      it("reverts without tokens approved for staking", async function () {
        await expectRevert(
          this.liquidityFarming.stake(staker1Initial, { from: staker1 }),
      ...
      context("staker1 & 2 stake at the same time", function () {
        ...
        beforeEach(async function () {
          ...
          await this.lp.approve(this.liquidityFarming.address, staker1Initial, { from: staker1 });
          await this.lp.approve(this.liquidityFarming.address, staker2Initial, { from: staker2 });
          tx1 = await this.liquidityFarming.stake(staker1Initial, { from: staker1 });
          tx2 = await this.liquidityFarming.stake(staker2Initial, { from: staker2 });
        });
        ...
        it("has expected total stake after multiple stakes", async function () {
          expect(await this.liquidityFarming.totalStakeLp()).to.be.bignumber.equal(
            staker1Initial.add(staker2Initial)
          );
        });

        it("has expected staker1 stake", async function () {
          expect(await this.liquidityFarming.userStakeLp(staker1)).to.be.bignumber.equal(staker1Initial);
        });

        it("has expected staker2 stake", async function () {
          expect(await this.liquidityFarming.userStakeLp(staker2)).to.be.bignumber.equal(staker2Initial);
        });
        ...
        it("has expected first stake time", async function () {
          ...
          expect(await this.liquidityFarming.firstStakeTime()).to.be.bignumber.equal(firstStakeTime);
        });

        it("has two total stakers", async function () {
          expect(await this.liquidityFarming.totalStakers()).to.be.bignumber.equal("2");
        });
        ...
        context("staker1 calls payout at half time", function () {
          ...
          beforeEach(async function () {
            ...
            reward1 = await this.liquidityFarming.payout.call(staker1, { from: staker1 });
            tx = await this.liquidityFarming.payout(staker1, { from: staker1 });
          });
          ...
          it("contract has staker1 claimed rewards", async function () {
            expect(await this.liquidityFarming.userClaimedRewards(staker1)).to.be.bignumber.equal(reward1);
          });
          ...
          context("advance to after staking ends", function () {
            ...
            it("reverts when staking after end time", async function () {
              ...
              await this.lp.approve(this.liquidityFarming.address, staker1Initial, { from: staker3 });
              await expectRevert(
                this.liquidityFarming.stake(staker1Initial, { from: staker3 }),
            ...
            it("reverts when payout/restake after end", async function () {
              await expectRevert(
                this.liquidityFarming.payout(staker2, { from: staker2 }),
            ...
            context("request withdraw for staker 2", function () {
              ...
              beforeEach(async function () {
                reward = (await this.liquidityFarming.withdraw.call(staker2, { from: staker2 })).reward;
                tx = await this.liquidityFarming.withdraw(staker2, { from: staker2 });
              });
              ...
              it("contract has staker2 claimed rewards", async function () {
                expect(await this.liquidityFarming.userClaimedRewards(staker2)).to.be.bignumber.equal(reward);
              });

              it("has one total staker", async function () {
                expect(await this.liquidityFarming.totalStakers()).to.be.bignumber.equal("1");
              });
              ...
              context("request withdraw for staker 1", function () {
                ...
                beforeEach(async function () {
                  reward2 = (await this.liquidityFarming.withdraw.call(staker1, { from: staker1 })).reward;
                  tx = await this.liquidityFarming.withdraw(staker1, { from: staker1 });
                });
                ...
                it("contract has no remaining rewards", async function () {
                  expect(await this.sarco.balanceOf(this.liquidityFarming.address)).to.be.bignumber.closeTo("0", "20000000");
                });

                it("contract has no remaining stakes", async function () {
                  expect(await this.lp.balanceOf(this.liquidityFarming.address)).to.be.bignumber.equal("0");
                });
              });
            });
          });
